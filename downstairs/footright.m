...
%% gait
    for t = 0 : stepTime : timeAll
    	...
    	if (phase2 < t && t <= phase2 + singleLegTime / 2.0)
            ...
            y0      = startDistance - stepLength * (1 - singleSwingCoefficient) * phase2TimeLine;
            z0      = -totalLeglength + crouchLength + liftLegLength * sin(M_PI * phase2TimeLine);
        ...
        if(phase3 < t && t <= phase3 + singleLegTime / 2.0)
            ...
            x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
            y0      = startDistance - stepLength * (1 - singleSwingCoefficient);
            z0      = -totalLeglength + crouchLength;
            Alpha   = AlpRight0 * sin(M_PI / singleLegTime * (t - phase2));
            Beta    = BetRight0 * sin(M_PI / singleLegTime * (t - phase2));
            Gamma   = GamRight0 * sin(M_PI / singleLegTime * (t - phase2));
        ...
        for i = 0 : 1 : stepNumbers
            ...
            if(phase5 < t && t <= phase5 + singleLegTime / 2.0)
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance + (stepLength * singleSwingCoefficient) * phase5TimeLine;
            ...
            if (phase6 < t && t <= phase6 + singleLegTime / 2.0 )
                ...
                z0      = -totalLeglength + crouchLength + stairHeight * sin(M_PI / 2 * phase6TimeLine)- liftComLength * sin(M_PI * (t-phase5) /singleLegTime);
                Alpha   = 0;
                Beta    = 0;
                Gamma   = 0;
                Alpha2  = AlpRight1 * sin(M_PI * 2 / singleLegTime * (t - phase5));
            ...
            if(phase8 < t && t <= phase8 + singleLegTime / 2)
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance + stepLength - stepLength * (2-singleSwingCoefficient) * phase8TimeLine;
                z0      = -totalLeglength + crouchLength + stairHeight + liftLegLength * sin(M_PI * phase8TimeLine);
                Alpha   = 0;
                Beta    = BetRight0 * sin(M_PI / cycleTime * (t - phase7));
            ...
            if(phase9 < t && t <= phase9 + singleLegTime / 2)
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance - stepLength *(1-singleSwingCoefficient);
                z0      = -totalLeglength + crouchLength + stairHeight - stairHeight * sin(M_PI / 2 * phase9TimeLine);
                Alpha   = 0;
