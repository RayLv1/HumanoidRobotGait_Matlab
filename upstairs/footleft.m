function []= footleft()
    ...
    for t = 0 : stepTime : timeAll
        ...
        if(phase3 < t && t <= phase3 + singleLegTime / 2.0)
            ...
            x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
            y0      = startDistance + stepLength * singleSwingCoefficient * phase3TimeLine;
            z0      = - totalLeglength + crouchLength;
            Alpha   = 0;
            Beta    = 0;
            Gamma   = 0;
        ...
        for i = 0 : 1 : stepNumbers
            ...
            if(phase4 < t && t <= phase4 + doubleLegTime)
                ...
                z0      = - totalLeglength + crouchLength - liftComLength * sin(M_PI * phase4TimeLine);
                Alpha   = 0;
                ...
                zcom    = totalLeglength - crouchLength + i * stairHeight * 2  + liftComLength * sin(M_PI * phase4TimeLine);
            end
            ...
            if(phase5 < t && t <= phase5 + singleLegTime / 2.0)
                ...
                z0      = - totalLeglength + crouchLength + liftLegLength * sin(M_PI * (t - phase5) / singleLegTime) + stairHeight * sin(M_PI_2 * phase5TimeLine) ;
                Alpha   = 0;
                Beta    = BetLeft0 * sin(M_PI / cycleTime * (t - phase4));
                ...
                Flag    = 0;
                xcom    = x0;
                ycom    = startDistance + i * 2 * stepLength + stepLength ;
                zcom    = totalLeglength - crouchLength + stairHeight * sin(M_PI_2 * phase5TimeLine) + i * stairHeight * 2  + liftComLength * sin(M_PI * (t - phase5) / singleLegTime);
            end
            ...
            if (phase6 < t && t <= phase6 + singleLegTime / 2.0 )
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance + stepLength -(stepLength * (2-singleSwingCoefficient)) * phase6TimeLine;
                z0      = - totalLeglength + crouchLength + stairHeight + liftLegLength * sin(M_PI * (t - phase5) / singleLegTime);
                Alpha   = 0;
                Beta    = BetLeft0 * sin(M_PI / cycleTime * (t - phase4));
                Gamma   = GamLeft0 * sin(M_PI / (singleLegTime) * (t - phase5));
                Alpha2  = 0;
                ...
                Flag    = 0;
                xcom    = x0;
                ycom    = startDistance + i * 2 * stepLength + stepLength + (stepLength * singleSwingCoefficient) * phase6TimeLine;
            ...
            if(phase7 < t && t <= phase7 + doubleLegTime)
                ...
                z0      = - totalLeglength + crouchLength + stairHeight - liftComLength * sin(M_PI * phase7TimeLine);
                Alpha   = 0;
                Beta    = 0;
                Gamma   = 0;
                Alpha2  = 0;
                ...
                zcom    = totalLeglength - crouchLength + stairHeight + i * stairHeight * 2 + liftComLength * sin(M_PI * (t - phase7) / doubleLegTime);
            end
            ...
            if(phase8 < t && t <= phase8 + singleLegTime / 2)
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance ;
                z0      = - totalLeglength + crouchLength + stairHeight - stairHeight * sin(M_PI_2 * phase8TimeLine) - liftComLength * sin(M_PI * (t-phase8) /singleLegTime);
                Alpha   = 0;
                Beta    = 0;
                Gamma   = 0;
                ...
                Flag    = 1;
                xcom    = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                ycom    = startDistance + i * 2 * stepLength + stepLength * 2 ;
                zcom    = totalLeglength - crouchLength + stairHeight + i * stairHeight * 2 + stairHeight * sin(M_PI_2 * phase8TimeLine) + liftComLength * sin(M_PI * (t-phase8) /singleLegTime);
            end
            ...
            if(phase9 < t && t <= phase9 + singleLegTime / 2)
                ...
                x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                y0      = startDistance + stepLength * singleSwingCoefficient * phase9TimeLine;
                ...
                Flag    = 1;
                xcom    = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
                ycom    = startDistance + i * 2 * stepLength  + stepLength * 2 + stepLength * singleSwingCoefficient * phase9TimeLine;
        ...
        if(phase10 < t && t <= phase10 + doubleLegTime)
            ...
            z0      = - totalLeglength + crouchLength - liftComLength * sin(M_PI * phase10TimeLine);
            Alpha   = 0;
            ...
            zcom    = totalLeglength + stepNumbers * 2 * stairHeight - crouchLength + liftComLength * sin(M_PI * phase10TimeLine);  
        end
        ...
        if(phase11 < t && t <= phase11 + singleLegTime / 2)
            ...
            x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
            y0      = startDistance + stepLength ;
            z0      = - totalLeglength + crouchLength + liftLegLength * sin(M_PI * (t - phase11) / singleLegTime) ;
            Alpha   = 0;
            Beta    = BetLeft0 * sin(M_PI*2 / cycleTime * (t - phase10));
            ...
            zcom    = totalLeglength + stepNumbers * 2 * stairHeight - crouchLength + liftComLength * sin(M_PI * ( t- phase11) /singleLegTime)+ stairHeight * sin(M_PI_2 * phase11TimeLine);
        end
        phase12 = phase11 + singleLegTime / 2;
        if(phase12 < t  && t <= phase12 + singleLegTime / 2.0)
            phase12TimeLine = (t - phase12) / (singleLegTime / 2.0);
            x0      = swingLength * sin(M_PI * (t - phase2 + doubleLegTime / 2) / cycleTime);
            y0      = startDistance  + stepLength - stepLength * phase12TimeLine;
            z0      = - totalLeglength + crouchLength + liftLegLength * sin(M_PI * (t - phase11) / singleLegTime);
            Alpha   = 0;
            Beta    = BetLeft0 * sin(M_PI*2 /  cycleTime * (t - phase10));
            Gamma   = GamLeft0 * sin(M_PI / (singleLegTime) * (t - phase11));
        ...
        if(phase13 < t && t <= phase13 + crouchTime)
            ...
            zcom    = totalLeglength + stepNumbers * 2 * stairHeight + stairHeight - crouchLength + crouchLength * sin(M_PI_2 * phase13TimeLine);
        end
